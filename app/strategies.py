        ...
        if df.empty:
            ...
            if strategy:
                ...
                def progress_callback(current_step, total_steps, current_time, last_reason):
                    # Only update UI every N steps to avoid slowdown, unless it's a signal
                    if current_step % 5 == 0 or last_reason != "Hold/No Signal":
                        progress = int((current_step / total_steps) * 100)
                        progress_bar.progress(progress)
                        status_text.text(f"Processing: {current_time} ({current_step}/{total_steps})")
                        if last_reason and last_reason != "Hold/No Signal":
